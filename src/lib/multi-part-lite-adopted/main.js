...
class MultipartLite extends CombinedStream {
  ...
  [generate](field, value, { filename, contentType }) {
    ...
    if (isBuffer(value) || isStream(value) || isHTTPStream(value) || isVinyl(value)) {
      ...
      // trimming the path has already been done by basePathConverter from pinata sdk 
      const file = filename; // this.getFileName(filename ? { filename } : value, this.opts.defaults);
